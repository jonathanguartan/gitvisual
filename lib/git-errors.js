...
const PATTERNS = [
  ...
  {
    // Push rechazado: rama remota tiene commits que el local no tiene (non-fast-forward)
    test: e =>
      e.message.includes('non-fast-forward') ||
      (e.message.includes('rejected') && e.message.includes('failed to push')) ||
      e.message.includes('Updates were rejected because the tip of your current branch is behind'),
    format(e) {
      const m = e.message.match(/-> ([^\s]+)\s+\(non-fast-forward\)/);
      const branch = m ? m[1] : 'la rama remota';
      return `Push rechazado: "${branch}" en el servidor tiene cambios que aún no tienes en local.\nHaz pull primero para integrarlos y luego vuelve a hacer push.`;
    },
  },
];
