...
// En entorno Electron empaquetado, __dirname apunta dentro del asar (solo lectura).
// Usamos app.getPath('userData') para que la config sea persistente y escribible.
function resolveConfigPath() {
  try {
    const { app } = require('electron');
    if (app?.getPath) return path.join(app.getPath('userData'), 'app-config.json');
  } catch (_) {}
...
// Claves que se pueden sobreescribir por repositorio
const REPO_KEYS = ['rebaseOnPull', 'autoStash', 'diffContext', 'logLimit', 'mainBranch'];
...
function saveConfig(config) {
  ...
  // Per-repo overrides: merge dict by path, no overwrite unrelated repos
  if (safeInput.repos && typeof safeInput.repos === 'object' && !Array.isArray(safeInput.repos)) {
    merged.repos = sanitize(safeInput.repos);
  }
...
/**
 * Devuelve la config global fusionada con los overrides del repositorio indicado.
 * Las rutas que usan ajustes operativos (pull, diff, log) deben llamar a esta función.
 */
function loadRepoConfig(repoPath) {
  const global = loadConfig();
  if (!repoPath) return global;
  const overrides = global.repos?.[repoPath] || {};
  return { ...global, ...overrides };
}

/**
 * Guarda overrides para un repositorio concreto.
 * Solo persiste las claves permitidas (REPO_KEYS).
 */
function saveRepoConfig(repoPath, partial) {
  if (!repoPath) return;
  const current  = loadConfig();
  const safe     = sanitize(partial);
  const existing = current.repos?.[repoPath] || {};
  const updated  = { ...existing };
  for (const key of REPO_KEYS) {
    if (safe[key] !== undefined) updated[key] = safe[key];
  }
  const repos = { ...(current.repos || {}), [repoPath]: updated };
  saveConfig({ repos });
}

/**
 * Elimina todos los overrides de un repositorio (vuelve a usar la config global).
 */
function clearRepoConfig(repoPath) {
  if (!repoPath) return;
  const current = loadConfig();
...
/**
 * Devuelve los overrides activos para un repo (objeto vacío si no hay ninguno).
 */
function getRepoOverrides(repoPath) {
  if (!repoPath) return {};
  return loadConfig().repos?.[repoPath] || {};
}

module.exports = { loadConfig, saveConfig, loadRepoConfig, saveRepoConfig, clearRepoConfig, getRepoOverrides };
